...
async function getListData(req, res) {
  ...
  if (totalRows) {
    ...
    const sql = `SELECT * FROM \`product_sake\` ${sqlWhere} ORDER BY pro_id DESC LIMIT ${
      perPage * (page - 1)
    },${perPage}`;
...
router.post("/add", async (req, res) => {
  ...
  const sql =
    "INSERT INTO `product_sake`( `pro_name`, `pro_stock`, `pro_selling`, `pro_intro`, `pro_condition`, `format_id`, `pro_img`, `pro_creat_time`) VALUES(?, ?, ?, ?, ?, ?, ?, NOW())";
